...
from jev.core.compare import ComparisonReport, compare, comparison_plan
from jev.core.errors import JevError
from jev.core.evaluation import EvalReport, threshold_stats
from jev.core.files import read_text
from jev.core.jobs import BatchService, JobReport
...
from jev.core.spending import SpendEstimate, SpendScope
...
def display_report(report: JobReport, machine: bool) -> None:
    ...
    if machine:
        ...
        if report.error:
            ...
            console.print(Text("Inspect per-call errors with jev history --status failed --json."))
...
def show_evaluation(job_id: str, json_output: JsonFlag = False) -> None:
    ...
    if report.kind != "eval":
        raise JevError(
            "not_an_eval", "That job is a batch run.", "Inspect it with jev batch --json."
        )
    ...
    if resume:
        ...
        if not template or input_path is None or output is None:
            raise JevError(
                ...
                "Use jev batch TEMPLATE --input data.jsonl --output results.jsonl.",
            )
        design = wb.templates.load(template)
...
def compare_command(
    ...
    if left is None or right is None or (state is None) == (text is None):
        raise JevError(
            ...
            "Use jev compare LEFT RIGHT --state file.json or --text 'state'.",
        )
