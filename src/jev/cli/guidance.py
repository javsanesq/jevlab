...
from jev.cli.common import JsonFlag, console, emit, guarded, launch, workbench
from jev.core.config import data_directory
from jev.core.demo import load_demo
from jev.core.errors import JevError
from jev.core.guidance import GLOSSARY, WELCOME, explain_answer
...
def tour(json_output: JsonFlag = False) -> None:
    ...
    steps = [
        {
            ...
            "text": "An API key is a private password for your provider account. Run jev config "
            "to save a TypeSafe key securely in macOS Keychain. You can skip this for the demo.",
        },
        {
            "title": "Try the free recorded demo",
            "text": "Run jev demo. The teaching answers are replayed from a bundled file; "
    ...
    if json_output:
        ...
        console.print(Text("Run jev tour in an interactive terminal to follow the tour."))
...
def glossary(
    ...
    """Read plain explanations and examples of the words used in jev."""
    entries = GLOSSARY
    if term:
        entries = {
        ...
        if not entries:
            raise JevError(
                ...
                "Run jev glossary to see all available words.",
            )
    if json_output:
        emit(entries)
