...
from jev.cli.spending import confirm_spend, interactive
from jev.coach.service import Coach, CoachResult
from jev.core.content import export_dataset, lesson, lessons, pattern, patterns, starter
...
from jev.core.models import ConfidenceGate, Gate, NoulGate, Template
from jev.core.pricing import format_cost
from jev.core.spending import SpendEstimate, estimate_coach
from jev.core.templates import dump_template, fork_template
...
def start_lesson(lesson_id: str, name: str | None = None, json_output: JsonFlag = False) -> None:
    ...
    emit(data) if json_output else console.print(
        Text(
            f"Saved draft {design.name}. Edit it with jev templates edit {design.name}.\n"
            f"{item.exercise}"
        )
    )
    ...
    if plan.requires_confirmation and not yes:
        ...
        if not yes:
            raise JevError(
                ...
                "Review jev learn plan, then pass --yes to authorize.",
            )
    ...
    if json_output:
        ...
        if report.status != "completed":
            envelope["error"] = JevError(
                ...
                f"Inspect the saved attempt with jev learn inspect {report.id} --json.",
                4,
            ).as_dict()
            ...
            if any("*" in gate_label(stats.gate) for stats in report.routing.values()):
                console.print(
                    Text(
                        ...
                        "Use jev learn inspect --json for exact values."
                    )
                )
...
def inspect_attempt(attempt_id: str, json_output: JsonFlag = False) -> None:
    try:
        ...
        raise JevError(
            ...
            "Use a full ID from jev learn progress --json. Cleanup may prune attempt details; "
