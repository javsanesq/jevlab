...
from jev.tui.fields import Field, Validator, required
...
class ErrorDetails(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        with VerticalScroll(classes="dialog help-dialog"):
            ...
            yield Static(
                "Use jev doctor --json when asking for help. Never share an API key.",
                classes="muted",
            )
...
class YamlEditor(ModalScreen[str | None]):
    ...
    def validate_yaml(value: str) -> str | None:
        ...
        from jev.core.templates import parse_template
