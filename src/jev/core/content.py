...
from jev.core.models import StrictModel, Template
...
def patterns() -> list[Pattern]:
    root = files("jev.resources").joinpath("library")
...
def pattern(name: str) -> Pattern:
    for entry in patterns():
        if entry.id == name:
            return entry
    raise JevError("pattern_not_found", "Unknown library pattern.", "Run jev library --json.")
...
def lessons() -> list[Lesson]:
    data = files("jev.resources").joinpath("lessons.json").read_text()
...
def lesson(name: str) -> Lesson:
    for item in lessons():
        if name == item.id or (name.isdigit() and item.id.startswith(f"{int(name):02d}-")):
            return item
    raise JevError("lesson_not_found", "Unknown lesson.", "Run jev learn --json for the track.")
