...
from jev.core.diagnostics import provider_fields, redact_body, redact_text
...
def translate_error(
    ...
    if isinstance(error, TypeSafeAPIError):
        ...
        if isinstance(error, TypeSafeAPIResponseValidationError):
            ...
            fix = (
                ...
                "jev doctor and report this response to TypeSafe before retrying."
            )
        elif status == 401 or any(
            x in evidence for x in ("invalid_api_key", "authentication_error")
        ):
            code, fix = "authentication", "Replace the TypeSafe API key in jev config, then retry."
        elif (
            any(
                x in evidence
                for x in (
            ...
            fix = (
                ...
                "in the template editor. Run jev doctor --online to list available models."
            )
        elif (
            any(
                x in evidence
            ...
            code, fix = (
                "not_found",
                (
                    "Run jev doctor --online to check available models and service access; report "
    ...
    return JevError(
        ...
        "Open technical details for the exception type, run jev doctor, and report the run ID.",
        4,
        details=details,
    )
