...
from jev.core.coach_models import CoachProvider
from jev.core.credentials import ENV_KEYS, Credentials
    ...
    for provider, estimate in zip(PROVIDERS, estimates, strict=True):
        ...
        try:
            ...
            blockers.append(
                JevError(
                    ...
                    f"Run jev config or use {ENV_KEYS[provider]} in environment mode.",
                    3,
                ).as_dict()
            )
        if installed is None:
            blockers.append(
                JevError(
                    "coach_dependency",
                    f"The {provider} SDK is not installed in this jev environment.",
        ...
        if key is None and not blockers_contain(
            ...
            blockers.append(
                JevError(
                    ...
                    "Run jev config to save the key or set "
