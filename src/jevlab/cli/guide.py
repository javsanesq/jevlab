...
from jevlab.cli.common import JsonFlag, console, emit, guarded
from jevlab.core.config import data_directory
from jevlab.core.errors import JevError
from jevlab.core.files import atomic_write

GUIDE_TITLE = "jevlab: A complete beginner's guide"
...
<title>jevlab: A complete beginner's guide</title>
<style>
...
def _editable_guide_path() -> Path | None:
    ...
    if (root / "src" / "jevlab" / "cli" / "guide.py").resolve() != module:
        return None
...
def load_guide() -> str:
    ...
    try:
        ...
        return files("jevlab").joinpath("resources", "GUIDE.md").read_text(encoding="utf-8")
    except (OSError, UnicodeError):
        raise JevError(
            "guide_unavailable",
            "The beginner's guide is missing or cannot be read in this installation.",
            "Reinstall jevlab from the current project, then run jevlab guide again.",
        ) from None
    ...
    if json_output:
        ...
        if not opened:
            raise JevError(
                ...
                f"Open {destination} in your browser, or run jevlab guide to read it here.",
            )
