...
def server_token_source() -> str:
    # An explicitly empty new setting must fail validation, not revive an old token.
    return "JEVLAB_SERVER_TOKEN" if "JEVLAB_SERVER_TOKEN" in os.environ else "JEV_SERVER_TOKEN"


def server_token() -> str:
    source = server_token_source()
    ...
    if (
        ...
        raise JevError(
            ...
            f"{source} needs 32–512 printable ASCII characters without whitespace. "
            "JEVLAB_SERVER_TOKEN is the current setting name.",
        ...
        if authenticated:
            ...
            if len(headers) != 1 or not hmac.compare_digest(headers[0].encode(), expected.encode()):
                return failure(
                    ...
                    "Send Authorization: Bearer with your JEVLAB_SERVER_TOKEN.",
                    401,
                )
        return None
    ...
    async def run(self, request: Request) -> Response:
        ...
        try:
            ...
            return failure(
                ...
                "Check jevlab doctor and saved history before retrying.",
                500,
            )
        finally:
            self.active -= 1
