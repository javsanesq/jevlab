...
def coach_error(error: Exception, *, provider: str = "", secrets: Iterable[str] = ()) -> JevError:
    ...
    code, summary, fix, retryable = (
        ...
        "Run jevlab doctor --coach; if the checks pass, report this failure with its error code.",
        False,
    )
    if status == 401:
        code, summary, fix = (
            ...
            "Replace this provider's key with jevlab config "
        ...
        code, summary, fix = (
            ...
            "Check this provider's API model identifier in jevlab config and its account access.",
        )
    elif status == 402 or (
        status in (400, 429)
        and any(
