...
## Developer workbench, Phase 3 — distribution handoff (2026-09-22)

| ID | Decision | Reason |
| --- | --- | --- |
| D100 | Prepare PyPI publication as a manually dispatched workflow on a verified version tag, with unprivileged build/test and a separate `pypi` environment job using OIDC. Keep index setup and upload owner-run; do not auto-publish on GitHub release. | A source release is already available, but the owner needs a reproducible package path without long-lived upload secrets or accidental publication. A protected environment must be configured before the first dispatch. |
...
## Developer workbench, Phase 5 — developer loop (2026-09-24)

The owner reviewed an audit, chose developers integrating Jev as the audience and
real adoption as the goal, approved relaxing spending confirmations, and asked for
the audit's findings to be implemented.

| ID | Decision | Reason |
| --- | --- | --- |
| D106 | One consent rule for every paid workflow (batch, eval, compare, lesson grading, coach, local server): a known estimate at or below `confirm_cost_usd` starts with a notice; a larger or unknown estimate asks in a terminal and needs `--yes` in scripts. Retire `confirm_batch_cost`/`confirm_eval_cost` (older configs still load; the keys are dropped on save). Starting the server no longer asks. Supersedes D54, D71 and D75. | At the published $0.042 per million input tokens, typical jobs cost cents; prompting before every job added friction without protecting meaningful amounts. Scripts keep the same unattended budget gate, now reached less often. |
| D107 | Estimate `jev-latest` at the rate of the version it resolved to on the price date (`jev-1.13.0`, per RESEARCH.md), recording `priced_as`. Default jobs to eight workers and ten starts/second, and halve the start rate after a provider 429 while honoring its retry delay. Supersedes D27's defaults. | An alias previously made every estimate unknown, forcing prompts and server 409s. Ten starts/second is half of the last researched published limit (1,200/minute), not re-verified live; a 10,000-row eval no longer takes 83 minutes at the old two/second. |
| D108 | Jobs and the local server look up the key once and reuse one pooled SDK client; single runs close their own client. The shared lookup happens before a run's call timer. | Per-row key lookups and TLS handshakes were pure overhead for a 70–500 ms model. Keeping lookup outside the timer preserves D77: a slow lookup is a credential error with no request sent, not an uncertain provider timeout. |
| D109 | Add a validated `base_url` setting (HTTPS, or HTTP only for loopback; no credentials, query or fragment). Keep ignoring `TYPESAFE_BASE_URL`. Exported modules accept an injected SDK client. | Organizations need proxies and alternative roots, but an inherited environment variable must not redirect the key (D03's concern). Services calling an export many times need connection reuse. |
| D110 | Add `eval check` (run, then compare with a baseline and/or minimum accuracy; exit 5 for quality, 4 for incomplete calls) with a pre-run pairing check, and `--save-baseline` on `eval run`/`eval check`. Ship a GitHub Actions example. | The regression workflow required a Python one-liner and three commands. CI needs one command whose exit code distinguishes an outage from a regression, and an unpaired dataset should fail before spending. |
| D111 | Report 95% Wilson score intervals for accuracy and automated accuracy. `eval tune --target-accuracy` recommends the lowest observed cutoff meeting the target (Noul: each side separately; `--conservative`: lower bound must meet it). | Small eval sets are the norm; a point estimate alone overstates certainty. The key product question is which threshold reaches a target accuracy and at what coverage. Recommendations remain tuning evidence, verified separately under D96. |
| D112 | For JSON-format templates, CSV columns other than `id`/`expected.*` form the state, with dotted names nesting. Reject conflicting or empty segments and label-like column names. Accept Noul CSV labels in any case; list valid Choice options in errors. | Real exports are multi-column tables. A mistyped label column must not silently become model input. Label names are template content, not private input. |
| D113 | Human errors print `Error`/`Next` unless an external cause is known, which keeps What happened/Why/Next (refines D56). Reject `.jsonl`/`.csv` as `--state`; explain the expected JSON shape on invalid state. Group CLI help by developer workflow; give every command help text; use distinct warning/error colors (refines D12); remove the internal `phase` field from root JSON. | Repeating "This action could not be completed" added no information. Sending a dataset as one state was an easy, billable mistake. |
| D114 | Run tests in parallel (`pytest-xdist`, `make test`) with a 120-second per-test timeout (`pytest-timeout`). Measure the rate limiter at its grant times. | The suite fell from about four minutes to one. A hung test (for example an unexpectedly started server) must fail rather than stall CI. Timing assertions must not depend on scheduler jitter under parallel load. |

No live provider call was made for this phase. Rate limits and alias resolution
come from the dated research record, not a new live check.
